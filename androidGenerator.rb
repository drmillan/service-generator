...
class AndroidGenerator
  def generate(protocol, projectName, packageName, aVersion, aOutput, staticPackage)
    ...
    parameters['staticPackage']=staticPackage
    ...
    originStatic = "templates/android/"+aVersion+"/static/"
    outputStatic = aOutput+'/'+staticPackage.gsub('.', '/')

    # -staticPackage com.salsalol.service
    # FileUtils.mkdir_p outputStatic
    # FileUtils.cp_r Dir.glob("templates/android/"+aVersion+"/static/*"), outputStatic

    Dir.glob("#{originStatic}**/*") do |f|
      if File.file?(f)
        source_dir_name = File.dirname(f).sub!(originStatic, '')
        new_name = outputStatic + '/' + source_dir_name + '/' + File.basename(f).gsub('.mustache', '')
        # Process mustache
        res=Mustache.render(File.open(f).read, parameters)
        File.open(new_name, 'w') { |file| file.write(res) }
      elsif File.directory?(f)
        target = outputStatic + '/' + f.gsub(originStatic, '') + '/'
