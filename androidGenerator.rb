...
class AndroidGenerator
  def generate(protocol,projectName,packageName,aVersion,aOutput)
    ...
    baseDTODir=aOutput+"/src/"+packageName.gsub('.','/')+"/model/dto/base/"
    ...
    dtoDir=aOutput+"/src/"+packageName.gsub(".","/")+"/model/dto/"
    ...
    ############ DAO generation
    daoDir=aOutput+"/src/"+packageName.gsub('.','/')+"/model/dao/"
    ...
    puts 'Services'
    puts '--------'
    logicBaseDir=aOutput+"/src/"+packageName.gsub(".","/")+"/logic/base"
    logicDir=aOutput+"/src/"+packageName.gsub(".","/")+"/logic"
    ...
    protocol.services.keys.each do |serviceKey|
      protocol.services[serviceKey].messages.each do |message|
        ...
        taskFileDir=aOutput+"/src/"+packageName.gsub(".","/")+"/tasks/"+serviceKey.downcase
    ...
    helperFile=aOutput+"/src/"+packageName.gsub(".","/")+"/logic/"+projectName+"Helper.java"
    ...
    FileUtils.cp_r("templates/android/"+aVersion+"/static/src",aOutput)
